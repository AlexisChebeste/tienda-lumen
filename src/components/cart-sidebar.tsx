...
export default function CartSidebar ({isOpen, onClose} : CartSidebarProps) {
    const {totalItems, totalPrice, items, updateQuantity, removeItem} = useCart()
    return (
        <>
            {isOpen && (
                <div className="fixed inset-0 z-40 bg-black/30 backdrop-blur-x transition-opacity" onClick={onClose} />
            )}
                ...
                <div className="flex h-full flex-col">
                                ...
                                {items.map((item) => {
                                    return(
                                        <div key={`${item.id}-${item.size}-${item.color}`} className="flex gap-2">
                                            <div className="relative w-28 overflow-hidden bg-muted">
                                                <Image
                                                    src={item.image}
                                                    alt="Product"
                                                    fill
                    ...
                    {items.length > 0 && (
                        <div className=" flex flex-col gap-4 border-t border-border px-6 py-5">
                            ...
                            <button className="flex items-center justify-center w-full p-4 bg-stone-950 text-white hover:bg-stone-800 transition-colors duration-200 cursor-pointer font-semibold uppercase">
                                Finalizar Compra
                            </button>
                        </div>
                    )}
